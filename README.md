...
- The approved artwork is the Lunar Shared Horizon scene. `public/images/lunar-horizon-v2/` contains pre-encoded AVIF/WebP sizes up to 3840px, with a matching mobile crop and an inline preview. Responsive preloads fetch only the format and size needed. Regenerate the assets and `lib/hero-artwork.json` with `node scripts/generate-hero-artwork.mjs`; bump the output directory version when changing artwork or encoding settings because these URLs are cached immutably. The retained `public/images/lunar-shared-horizon-v1.webp` master was resampled from a 1672 × 941 generated original, so the 4K delivery dimensions do not imply native 4K detail.
